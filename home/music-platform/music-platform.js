...
function initEventListeners() {
    ...
    sourceBtns.forEach(t = >{
        t.addEventListener("click", () = >{
            sourceBtns.forEach(e = >{
                e.classList.remove("bg-primary", "text-white", "border-primary"),
                e.classList.add("bg-white", "text-gray-700", "border-gray-200")
            }),
            t.classList.remove("bg-white", "text-gray-700", "border-gray-200"),
    ...
    progressSlider.addEventListener("input", () = >{
        const e = parseInt(progressSlider.value),
        t = e / 100 * audioPlayer.duration;
        audioPlayer.currentTime = t
    }),
    volumeSlider.addEventListener("input", () = >{
        const e = parseInt(volumeSlider.value) / 100;
        audioPlayer.volume = e,
        volumeBar.style.width = `$ {
            100 * e
        } % `,
        0 === e ? volumeBtn.innerHTML = '<i class="fa fa-volume-off"></i>': volumeBtn.innerHTML = '<i class="fa fa-volume-up"></i>'
    }),
...
function performSearch() {
    ...
    2e3);
    try {
        const t = API_BASE_URLS[currentSource],
        a = new URLSearchParams({
            key: API_KEY,
            type: "json"
        });
        "wyy" === currentSource ? a.append("gm", e) : a.append("msg", e);
        const r = `$ {
            t
        } ? $ {
            a.toString()
...
function processSearchResults(e) {
    if (loading.classList.add("hidden"), 0 === e.length) return void noResults.classList.remove("hidden");
    searchResults = e,
